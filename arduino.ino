...
int readADC_sel(int read_time, int read_set_time, int read_delay, bool boost_bool) {
    ...
    // set cap bottom electrode voltage. CODR = HIGH voltage, PIO_SODR = LOW (GND).
    // Boost raises the read-Tr gate to Vcap + Vwwl, so boost=true must drive CODR
    // (HIGH); default (no boost) = cap bottom LOW so the gate sees Vcap only.
    PIOC->PIO_SODR  = N4_PINLIST[0];
    if (boost_bool) {
        PIOC->PIO_CODR  = N4_PINLIST[0];
    }
